...
`])));XE(kv||(kv=JE(["",""],["",""])),ZE);var Cv,kv;function nk(){return H.jsx(H.Fragment,{children:H.jsx(xP,{})})}var xv={};/**
 * @license
...
`,mM=Ve.span`
  ...
  font-weight: 900;
  margin-right: 10px;
`,G_=sI(Ju,"icecream","Mtu2EMz2fp8FKkItKQm5");function gM(){const t=ii.currentUser.uid,e=ii.currentUser.displayName,n=Al(),[r,i]=j.useState(!1),[s,o]=j.useState(!1),[l,u]=j.useState(!1),[c,d]=j.useState([]),[f,m]=j.useState([]),[E,A]=j.useState([]),[C,O]=j.useState([]),[I,v]=j.useState(""),[R,b]=j.useState(!1),F=async()=>{i(!0);try{const T=H_($_(Ju,"icecream"));await K_(T,Y=>{const Pe=Y.docs.map(rt=>rt.data());m(Pe[0].recipe),v(Pe[0].lastUser)})}catch(T){console.log(T)}finally{i(!1)}},P=T=>{const Y=[T,...E];A(Y)},_=()=>{o(!0);try{if(f.length!==E.length)return!1;for(let T=0;T<f.length;T++)if(f[T]!==E[T])return!1;k(),b(!0),setTimeout(()=>{b(!1)},1e3)}catch(T){console.log(T)}finally{A([]),S(),w(),o(!1)}},g=T=>{let Y=T.slice();for(let Pe=Y.length-1;Pe>0;Pe--){const rt=Math.floor(Math.random()*(Pe+1));[Y[Pe],Y[rt]]=[Y[rt],Y[Pe]]}return Y},w=async()=>{const T=g([0,1,2]);O(T);try{await Ld(G_,{recipe:T})}catch(Y){console.log(Y)}},S=async()=>{u(!0);try{const T=H_($_(Ju,"rank"),KL("score","desc"));await K_(T,Y=>{const Pe=Y.docs.map(rt=>rt.data());d(Pe)})}catch(T){console.log(T)}finally{u(!1)}},k=async()=>{if(!ii.currentUser)return;const T=sI(Ju,"rank",t),Y=await JL(T);if(await Ld(G_,{lastUser:e}),Y.exists()){const rt=(Y.data().score||0)+1;await Ld(T,{score:rt})}else await ZL(T,{score:1,userName:e||"Unknown User"})},N=async()=>{try{await ii.signOut(),n("/login")}catch(T){console.error("Error signing out: ",T)}};return j.useEffect(()=>{F(),S()},[]),j.useEffect(()=>{E.length===3&&_()},[E]),H.jsx(H.Fragment,{children:H.jsxs(nM,{children:[H.jsxs(rM,{className:R?"active":"",children:[e," 성공!!"]}),H.jsxs(iM,{children:[H.jsx(mM,{children:e}),H.jsx(sM,{onClick:()=>{N()},children:"로그아웃"})]}),H.jsx(oM,{children:"ICECREAM FACTORY"}),H.jsxs(aM,{children:[H.jsx(lM,{children:r?H.jsx(Vd,{children:"Loading..."}):H.jsxs(H.Fragment,{children:[H.jsx("p",{className:"title",children:"만들어주세요!"}),H.jsx("div",{className:"object",children:f.map((T,Y)=>H.jsx("div",{className:`${T===0?"strawberry":T===1?"choco":"mint"} ${Y===0?"one":Y===1?"two":"three"}`},Y))})]})}),H.jsx(uM,{children:s?H.jsx(Vd,{children:"Loading.."}):H.jsxs(H.Fragment,{children:[H.jsx("p",{className:"title",children:"나의 조합"}),H.jsx("div",{className:`${E.length===2?"second":E.length===3?"third":""} object`,children:E.map((T,Y)=>H.jsx("div",{className:`${T===0?"strawberry":T===1?"choco":"mint"} ${Y===0?"one":Y===1?"two":"three"}`},Y))})]})})]}),H.jsxs(cM,{children:[H.jsxs("div",{children:[H.jsx(Md,{className:"strawberry",onClick:()=>P(0)}),H.jsx(Md,{className:"choco",onClick:()=>P(1)}),H.jsx(Md,{className:"mint-choco",onClick:()=>P(2)})]}),H.jsx("span",{children:"SELECT ME!!!"})]}),H.jsxs(hM,{children:[H.jsxs("div",{className:"first-user",children:[H.jsx("p",{className:"title",children:"최고의 실력자"}),H.jsx("span",{className:"name",children:c.length>0?c[0].userName:"1등 없음!"})]}),l?H.jsx(Vd,{children:"Loading..."}):H.jsxs(dM,{children:[H.jsx("div",{className:"index",children:"기회가 있는 자"}),H.jsx(fM,{children:c.slice(1).map((T,Y)=>H.jsxs(pM,{children:[H.jsx("span",{children:T.userName}),H.jsx("span",{children:T.score})]},Y))})]})]})]})})}function yM(){return H.jsx("div",{children:"hello"})}const wI=Ve.div`
  display: flex;
  align-items: center;
  justify-content: center;
  flex-direction: column;
...
`;function TM(){const[t,e]=j.useState(!0),n=async()=>{await ii.authStateReady(),e(!1)};return j.useEffect(()=>{n()},[]),H.jsxs(H.Fragment,{children:[H.jsx(EM,{}),t?H.jsx(tk,{}):H.jsx(KP,{router:wM})]})}Fd.createRoot(document.getElementById("root")).render(H.jsx(Mn.StrictMode,{children:H.jsx(TM,{})}));
